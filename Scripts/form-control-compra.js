...
function validaCampos(){
    ...
    var verificado = 0;
      ...
      if (numeroTarjeta.length <= 0) {
        ...
        validaFalla(numeroTarjetaInput, 'Escribir numero de tarjeta valido');
      } else {
      ...
      } if (!/^(0[1-9]|1[0-9]|2[0-9]|3[1])\/(0[1-9]|[1-9][0-9])$/.test(fechaNacimiento)) {
      ...
      if (codigoSeguridad.length < 0) {
        ...
        validaFalla(codigoSeguridadInput, 'El código debe tener 3 o 4 numeros');
      } else {
...
const validaOk = (input) => {
    ...
    const aviso = document.getElementsByClassName('error-message');
}
